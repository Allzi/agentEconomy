...
bankN           = 10
duration        = 1000 :: Int
mrC1            = 0.5 -- when income
mrC2            = 1 -- when no income
rdinv1          = 0.1 -- for rich
rdinv2          = 0 -- for poor
wageGrowth      = 0.05
priceGrowth     = 0.1
...
--replases something with a random element of a list
...
updatePlan p (r:rs) prod = if (prod'^.pPrice < prod'^.pAC)
  ...
  where 
      prod'
        | (prod^.pGoods == Nothing)  && (prod^.pPrice < p)
...
hire pid = do
  ...
  where
    ...
    tryHire a@(wid, wage) = do
        ...
        if ((length (p^.pWorkers) < p^.pLDemand) 
            && (Nothing == (w^.wEmployer)))
            then do
